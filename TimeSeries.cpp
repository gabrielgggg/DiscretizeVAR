 ...
 */
vec discretizeNormal(vec grid, const double mean, const double sigma, bool cutTails)
{
...
DiscreteRV discretizeNormal(const double mean,
    const double sigma,
    const uword n_elem,
    const uword pmSigma, bool cutTails)
{
    DiscreteRV drv;
...
void DiscreteVAR::save(const std::string fname) const
{
    ...
    m_mc.transition().save(hdf5_name(fname, "DVAR/transitions", hdf5_opts::replace));
    m_var.intercept().save(hdf5_name(fname, "DVAR/varIntercept", hdf5_opts::replace));
...
void DiscreteVAR::impl(bool trimGrids, Method method)
{
    // Sizing
    m_size = m_var.size();
